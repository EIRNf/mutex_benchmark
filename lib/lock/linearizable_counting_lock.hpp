...
// =============================================================================
// SECTION 2.6: Heartbeat-Overtaking Counting Lock  (Design G)
//
// Refines Design F with the lesson from its measurement: registration is an
// ARRIVAL signal, but skipping is only profitable when the skipped-over
// thread is not RUNNING — and a registered waiter can itself be
// descheduled. Design F's window scan therefore gained no scheduling
// information and merely tied Seq/WF (ELEVATOR_SET_COMPARISON.md §7.7).
//
...
// Heartbeat-overtaking (Design G) — CAS only, same reasoning as Design F.
